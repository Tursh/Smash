...
{
    public static FrameOfAttack SimplePhysicalAttack(FrameDataPhysical frameDataPhysical)
    {
    ...
    public static FrameOfAttack ComplexPhysicalAttack(FrameDataPhysical[] framesDataPhysical)
    {
        return new FrameOfAttack(o =>
            {
                for (int i = 0; i < framesDataPhysical.Length; ++i)
                {
                    ...
                    GameObject hurtboxGameObject =
                        Instantiate(o.GetComponent<CharacterData>().Prefabs[framesDataPhysical[i].PrefabIndex],
                            o.transform);
                    CircleCollider2D circleCollider2D = hurtboxGameObject.GetComponent<CircleCollider2D>();
                    HurtboxComponent hurtboxComponent = hurtboxGameObject.GetComponent<HurtboxComponent>();
